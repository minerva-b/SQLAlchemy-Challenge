...
def welcome():
    ...
    return (
        ...
        f" </br>"
        f"<b>Dynamic API Routes:</b></br>"
        f" </br>"
        f"When searching for maximum, minimum, and average temperature use the following routes:"
        f" </br>"
        f"<li>For a single date, enter the date in this format (yyyy-mm-dd): /api/v1.0/<start></br>"
        f"<li>For a range of dates, enter in this format (yyyy-mm-dd/yyyy-mm-dd): /api/v1.0/<start>/<end></br>"
    )
...
def start(start):
    ...
    TMAX = session.query(func.max(Measurement.tobs)).filter(Measurement.date >= start).all()[0][0]
    TMIN = session.query(func.min(Measurement.tobs)).filter(Measurement.date >= start).all()[0][0]
...
def start_end(start, end):
    ...
    return (
        f'<b> Entered date range:</b> {start} to {end} <br/>'
        f' <br/>'
        f'Maximum Temperature: {TMAX} <br/>'
        f'Minimum Temperature: {TMIN} <br/>'
        f'Average Temperature: {round(TAVG, 1)}')
